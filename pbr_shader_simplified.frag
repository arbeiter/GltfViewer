...
in vec3 v_color;
in mat3 tbn;
uniform vec4 base_color_provided;
...
void main() {
  ...
  vec4 texCoordElem = texture2D(samp_tex, texCoord);
  vec3 baseColor = pow(texCoordElem.rgb, vec3(gamma));
  ...
  // gamma correction
  FragColor = vec4(result, texCoordElem.w);
}
